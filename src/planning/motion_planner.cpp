...
: params_(params)
{
    std::cout << "RADIUS " << params.robotRadius << std::endl; 
    std::cout << "RADIUS_ " << params_.robotRadius << std::endl; 
    setParams(params);
}
...
{   
    ...
    if(distances_.isCellInGrid(goalCell.x, goalCell.y))
    {
        ...
        return distances_(goalCell.x, goalCell.y) > params_.robotRadius + .015;
    }
